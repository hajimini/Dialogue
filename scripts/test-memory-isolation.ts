...
// 配置
const MEMORY_TEST_BASE_URL = process.env.BASE_URL || 'http://localhost:3000';
const MEMORY_TEST_USER_EMAIL = process.env.TEST_USER_EMAIL || 'admin@ai-companion.local';
const MEMORY_TEST_USER_PASSWORD = process.env.TEST_USER_PASSWORD || 'admin123456';
...
// 测试结果收集
const memoryTestResults: MemoryTestResult[] = [];

// 辅助函数：登录并获取 cookie
async function memoryTestLogin(): Promise<string> {
  console.log('🔐 登录测试账户...');

  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/auth/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      email: MEMORY_TEST_USER_EMAIL,
      password: MEMORY_TEST_USER_PASSWORD
    })
  });
...
// 辅助函数：获取人设列表
async function memoryTestGetPersonas(cookie: string): Promise<Array<{ id: string; name: string }>> {
  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/personas`, {
...
// 辅助函数：获取角色列表
async function memoryTestGetCharacters(cookie: string): Promise<Array<{ id: string; name: string }>> {
  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/characters`, {
    headers: { Cookie: cookie }
  });
...
// 辅助函数：创建新会话
async function memoryTestCreateSession(
  cookie: string,
  personaId: string,
  characterId: string
): Promise<string> {
  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/personas/${personaId}/sessions`, {
    method: 'POST',
    headers: {
      Cookie: cookie,
...
// 辅助函数：发送消息
async function memoryTestSendMessage(
  cookie: string,
  personaId: string,
  sessionId: string,
  message: string
): Promise<string> {
  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/chat`, {
    method: 'POST',
    headers: {
      Cookie: cookie,
      'Content-Type': 'application/json'
...
// 辅助函数：查询记忆
async function memoryTestGetMemories(
  cookie: string,
  personaId: string,
  characterId: string
  ...
  const response = await fetch(`${MEMORY_TEST_BASE_URL}/api/memories?${params}`, {
    headers: { Cookie: cookie }
  });
...
// 辅助函数：等待记忆存储
async function memoryTestWaitForMemoryStorage(ms: number = 3000): Promise<void> {
  console.log(`⏳ 等待 ${ms}ms 让记忆系统处理...`);
  await new Promise(resolve => setTimeout(resolve, ms));
}

// 测试场景 1：同用户 + 同人设 + 不同角色
async function memoryTestScenario1(cookie: string) {
  const scenario = MEMORY_TEST_SCENARIOS.scenario1;
  console.log(`\n📋 ${scenario.name}`);
  console.log('='.repeat(60));

  // 获取人设和角色
  const personas = await memoryTestGetPersonas(cookie);
  const characters = await memoryTestGetCharacters(cookie);
  ...
  // 步骤 1: 角色 A 对话
  console.log(`\n📝 步骤 1: 与角色 A 对话`);
  const sessionA = await memoryTestCreateSession(cookie, persona.id, characterA.id);
  console.log(`✓ 创建会话 A: ${sessionA}`);

  for (const fact of scenario.characterA.facts) {
    console.log(`  发送: "${fact.message}"`);
    const reply = await memoryTestSendMessage(cookie, persona.id, sessionA, fact.message);
    console.log(`  回复: "${reply.substring(0, 50)}..."`);
  }

  await memoryTestWaitForMemoryStorage();

  // 步骤 2: 角色 B 对话
  console.log(`\n📝 步骤 2: 与角色 B 对话`);
  const sessionB = await memoryTestCreateSession(cookie, persona.id, characterB.id);
  console.log(`✓ 创建会话 B: ${sessionB}`);

  for (const fact of scenario.characterB.facts) {
    console.log(`  发送: "${fact.message}"`);
    const reply = await memoryTestSendMessage(cookie, persona.id, sessionB, fact.message);
    console.log(`  回复: "${reply.substring(0, 50)}..."`);
  }
  ...
  // 查询角色 A 的记忆
  const memoriesA = await memoryTestGetMemories(cookie, persona.id, characterA.id);
  console.log(`✓ 角色 A 的记忆数量: ${memoriesA.length}`);

  // 查询角色 B 的记忆
  const memoriesB = await memoryTestGetMemories(cookie, persona.id, characterB.id);
  ...
  const replyA = await memoryTestSendMessage(cookie, persona.id, sessionA, '你还记得我最喜欢的颜色吗？');
  ...
  const replyB = await memoryTestSendMessage(cookie, persona.id, sessionB, '你还记得我最喜欢的颜色吗？');
...
// 主函数
async function memoryTestMain() {
  console.log('🚀 角色记忆隔离测试');
  console.log('='.repeat(60));
  console.log(`测试环境: ${MEMORY_TEST_BASE_URL}`);
  console.log(`测试用户: ${MEMORY_TEST_USER_EMAIL}`);
  console.log('');

  try {
    // 登录
    const cookie = await memoryTestLogin();

    // 运行测试场景
    await memoryTestScenario1(cookie);
    ...
    const passedTests = memoryTestResults.filter(r => r.passed).length;
    const totalTests = memoryTestResults.length;
    ...
    console.log('\n详细结果:');
    for (const result of memoryTestResults) {
      console.log(`\n${result.message}`);
    ...
    const report = {
      ...
      environment: MEMORY_TEST_BASE_URL,
      summary: {
        total: totalTests,
        passed: passedTests,
