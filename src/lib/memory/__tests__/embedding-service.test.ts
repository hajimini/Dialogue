 ...
 * 
 * These tests verify the EmbeddingService fallback logic and error handling.
 */
...
describe('EmbeddingService', () => {
  describe('Fallback Embedding Logic', () => {
    ...
    it('should handle mixed content', async () => {
      const text = 'User123 likes 猫 🐱 and dogs!';
      const embedding = await service.embed(text);
  ...
  describe('Error Handling and Fallback', () => {
    ...
    it('should not throw errors for any input', async () => {
      ...
      for (const input of testInputs) {
        await expect(service.embed(input)).resolves.toBeDefined();
      }
    });
  });
  ...
  describe('Provider Configuration', () => {
    ...
    it('should fall back for bge-m3 embedding (not yet implemented)', async () => {
      const config: EmbeddingServiceConfig = {
        provider: 'bge-m3',
        apiKey: 'test-key',
        model: 'BAAI/bge-m3',
  ...
  describe('Metrics Tracking', () => {
    ...
    it('should reset metrics correctly', () => {
      ...
      expect(initialMetrics.successful).toBe(0);
      expect(initialMetrics.failed).toBe(0);
    });
  });

  describe('Hash-based Fallback Algorithm', () => {
    ...
    it('should generate different hashes for different inputs', async () => {
      ...
      const embedding1 = await service.embed('Input 1');
      const embedding2 = await service.embed('Input 2');
    ...
    it('should distribute values across the vector', async () => {
      const config: EmbeddingServiceConfig = {
        provider: 'openai',
        apiKey: '',
        model: 'text-embedding-3-large',
      ...
      const embedding = await service.embed('Test input with some content');
    ...
    it('should handle case sensitivity', async () => {
      ...
      // Hash-based embeddings should be case-sensitive
      expect(embedding1).not.toEqual(embedding2);
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle single character', async () => {
      ...
      const embedding = await service.embed('a');
