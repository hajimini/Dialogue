 ...
 * 
 * These tests verify the RerankerService degradation logic and error handling.
 */
...
describe('RerankerService', () => {
  describe('None Provider (Default Fallback)', () => {
    ...
    it('should calculate scores correctly for 10 documents', async () => {
      const query = 'test';
      const documents = Array.from({ length: 10 }, (_, i) => `doc${i}`);

      const results = await service.rerank(query, documents);

      // Scores should decrease by 0.1 each
      for (let i = 0; i < 10; i++) {
        expect(results[i].relevanceScore).toBeCloseTo(1.0 - i * 0.1, 5);
      }
    });
  });

  describe('Degradation on API Errors', () => {
    it('should fall back to original order on Jina API error', async () => {
    ...
    it('should fall back to original order on Cohere API error', async () => {
      const config: RerankerServiceConfig = {
        provider: 'cohere',
        apiKey: '',
      };
    ...
    it('should not throw errors for any input when API fails', async () => {
      const config: RerankerServiceConfig = {
        provider: 'jina',
        apiKey: '',
      };
      ...
      const testCases = [
        ...
        { query: '中文查询', documents: ['文档1', '文档2'] },
        { query: 'long query '.repeat(100), documents: ['doc1'] },
      ];

      for (const { query, documents } of testCases) {
        await expect(service.rerank(query, documents)).resolves.toBeDefined();
      }
    });
  });
  ...
  describe('Result Format', () => {
    ...
    it('should preserve document content', async () => {
      ...
      expect(results[0].document).toBe('First document');
      expect(results[1].document).toBe('Second document');
      expect(results[2].document).toBe('Third document');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very long query', async () => {
      ...
      const query = 'long query '.repeat(1000);
      const documents = ['doc1', 'doc2'];
